...
int mqtt_connect(mqtt_broker_handle_t* broker)
{
	...
	fixed_header_len=fixedHeaderSize;
...
int mqtt_publish_with_qos_hex(mqtt_broker_handle_t* broker, const char* topic, const char* msg, uint8_t retain, uint8_t qos, uint16_t* message_id,int msg_len) 
{
    uint16_t topiclen = strlen(topic);
	uint16_t msglen = msg_len;//strlen(msg);
	...
	uint8_t var_header[1024]; // Topic size (2 bytes), utf-encoded topic
...
//	uint8_t fixed_header[fixedHeaderSize];
    ...
    var_header_len=topiclen+2+qos_size; //[topiclen+2+qos_size
   ...
   //uint8_t packet[sizeof(fixed_header)+sizeof(var_header)+msglen];
    packet_len=fixed_header_len+var_header_len+msglen;
	memset(packet, 0, packet_len);
	memcpy(packet, fixed_header, fixed_header_len);
...
int mqtt_subscribe(mqtt_broker_handle_t* broker, const char* topic, uint16_t* message_id,unsigned char qos) {
	uint16_t topiclen = strlen(topic);

   // uint8_t utf_topic[topiclen+3]; // Topic size (2 bytes), utf-encoded topic, QoS byte
    uint8_t utf_topic[1024]; // Topic size (2 bytes), utf-encoded topic, QoS byte
    int     utf_topic_len=0;
	...
	uint8_t fixed_header[] = {
		MQTT_MSG_SUBSCRIBE | MQTT_QOS0_FLAG, // Message Type, DUP flag, QoS level, Retain
		sizeof(var_header)+utf_topic_len
	};
    fixed_header[0]|=((qos&0x03)<<1);
	//uint8_t packet[sizeof(var_header)+sizeof(fixed_header)+sizeof(utf_topic)];
...
int mqtt_unsubscribe(mqtt_broker_handle_t* broker, const char* topic, uint16_t* message_id) {
	...
	uint8_t utf_topic[1024]; // Topic size (2 bytes), utf-encoded topic
    int     utf_topic_len=0;
	//uint8_t packet[sizeof(var_header)+sizeof(fixed_header)+sizeof(utf_topic)];
	uint8_t packet[4096];
    int     packet_len=0;
